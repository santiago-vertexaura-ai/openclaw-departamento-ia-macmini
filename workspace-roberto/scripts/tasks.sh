#!/bin/bash
# tasks.sh — Supabase CRUD helper para Roberto
...
priority_order() {
  # Supabase REST no soporta ORDER BY CASE, asi que ordenamos con jq
  jq 'sort_by(
...
case "$CMD" in
  ...
  resume)
    ...
    RESULT=$(curl -s "$API/agent_tasks?assigned_to=eq.roberto&status=eq.en_progreso&select=*" \
      "${AUTH_HEADERS[@]}" \
    ...
    FRESH=$(curl -s "$API/agent_tasks?assigned_to=eq.roberto&status=eq.pendiente&select=*" \
      "${AUTH_HEADERS[@]}" \
      -H "Prefer: return=representation")
  ...
  fetch)
    # Obtener tareas pendientes asignadas a roberto, ordenadas por prioridad
    RESULT=$(curl -s "$API/agent_tasks?assigned_to=eq.roberto&status=eq.pendiente&select=*" \
      "${AUTH_HEADERS[@]}" \
      -H "Prefer: return=representation")
  ...
  complete)
    ...
    if [[ "$TASK_TYPE" =~ ^(research|news_scan|report|youtube_analysis)$ ]]; then
      EXISTING=$(curl -s "$API/agent_docs?task_id=eq.$TASK_ID&select=id" \
        "${AUTH_WRITE[@]}")
      DOC_COUNT=$(echo "$EXISTING" | jq 'length' 2>/dev/null || echo "0")

      if [[ "$DOC_COUNT" == "0" ]]; then
        if [[ -f /tmp/roberto_doc.md ]]; then
          DOC_CONTENT=$(cat /tmp/roberto_doc.md)
        else
          SUMMARY=$(echo "$RESULT_JSON" | jq -r '.summary // "Sin resumen"')
          HIGHLIGHTS=$(echo "$RESULT_JSON" | jq -r '(.highlights // [])[]' 2>/dev/null || true)
          FILE_PATH=$(echo "$RESULT_JSON" | jq -r '.file_path // empty')
...
- $h"
          ...
          if [[ -n "$FILE_PATH" ]]; then
            # Intentar resolver path relativo y absoluto
            FULL_PATH="$FILE_PATH"
            [[ ! "$FILE_PATH" = /* ]] && FULL_PATH="$HOME/clawd/workspace-roberto/$FILE_PATH"
            if [[ -f "$FULL_PATH" ]]; then
              RAW_DATA=$(head -c 4000 "$FULL_PATH")
              DOC_CONTENT="$DOC_CONTENT
...
*Fecha: $NOW*"
    ...
    rm -f /tmp/roberto_doc.md
    set -e
    # === FIN AUTO-DOC ===

    # === VAULT INGEST (fire-and-forget) ===
    /Users/alfredpifi/clawd/scripts/vault-ingest.sh "$TASK_ID" "$TASK_TYPE" "$TASK_TITLE" "roberto" > /dev/null 2>&1 &
    ...
    if [[ -n "$TELEGRAM_BOT_TOKEN" && -n "$TELEGRAM_CHAT_ID" ]]; then
      SUMMARY_SHORT=$(echo "$RESULT_JSON" | jq -r '.resumen // .summary // "Tarea completada"' | head -c 200)
      MESSAGE="✅ *Roberto completó tarea*
...
${SUMMARY_SHORT}"
  ...
  fail)
    ...
    # Escapar error message para JSON seguro (puede contener comillas, newlines, etc.)
  ...
  doc)
    ...
    # El contenido markdown se lee desde stdin (pipe)
  ...
  urgent)
    ...
    PAYLOAD=$(python3 -c "import json; print(json.dumps({'title':'URGENTE: ' + json.loads(json.dumps('$TITLE')),'assigned_to':'alfred','task_type':'urgente','status':'pendiente','priority':'urgente','description':json.loads(json.dumps('$DESCRIPTION')),'created_by':'roberto'}))")
    ...
    if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$TELEGRAM_CHAT_ID" ]; then
      curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
        -d "chat_id=${TELEGRAM_CHAT_ID}" -d "text=URGENTE (Roberto): $TITLE" -d "parse_mode=Markdown" > /dev/null &
    fi
  ...
  *)
    ...
    echo "  fetch                              Obtener tareas pendientes (status=pendiente, assigned_to=Roberto)"
